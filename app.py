...
import time
from agents import build_reader_agent, build_search_agent, writer_chain, critic_chain, chat_answer_chain
...
def submit_question():
    if not st.session_state.chat_question.strip():
        return
    ...
    context = (
        ...
        f"Research Report:\n{st.session_state.results.get('writer', '')}"
    )
    answer = chat_answer_chain.invoke({
        "context": context,
...
with col_input:
    ...
    run_btn = st.button("⚡  Run Research Pipeline", on_click=start_pipeline, use_container_width=True)
...
if r:
    ...
    if "writer" in r and "critic" in r:
        st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
        st.markdown('<div class="section-heading">Ask Questions</div>', unsafe_allow_html=True)
        st.caption("Use the research report and gathered context to ask follow-up questions.")
        ...
        ask_btn = st.button("💬 Answer Question", on_click=submit_question, use_container_width=True)
        ...
        if st.session_state.chat_history:
            for item in reversed(st.session_state.chat_history):
                ...
                <div class="result-panel">
                    <div class="result-panel-title">Q</div>
                    <div class="result-content">{item['question']}</div>
                </div>
                """, unsafe_allow_html=True)
                st.markdown(f"""
                <div class="feedback-panel">
                    <div class="panel-label orange">A</div>
